...
/**
 * 根据 PR review 做后续跟进；若发现新自然语言任务，则执行并更新同一条 PR。
 * @param {{ client: ReturnType<typeof createGithubClient>, token: string, owner: string, repo: string, issueNumber: number }} params
 * @returns {Promise<void>}
 */
async function followUpPrReviews({
    client,
    token,
    owner,
    repo,
    ...
    const state = await findExistingDobBotState({ client, token, owner, repo, issueNumber })
    if (!state) return

    const reviews = await client.request("GET", `/repos/${owner}/${repo}/pulls/${state.prNumber}/reviews`, {
        token,
        query: { per_page: 100 },
    })
    if (!Array.isArray(reviews) || reviews.length === 0) return

    const latest = reviews[reviews.length - 1] as { state?: unknown; body?: unknown; id?: unknown }
    const reviewState = String(latest?.state ?? "")
    if (reviewState !== "CHANGES_REQUESTED") return
    const reviewAuthor = String((latest as { user?: { login?: unknown } | null })?.user?.login ?? "")
    if (!isAllowedMentionAuthor(reviewAuthor)) return
    logDob("runner.review_trigger", {
        issueNumber,
        prNumber: state.prNumber,
        reviewState,
        reviewAuthor,
        reviewBody: String(latest?.body ?? ""),
    })

    const instruction = extractDobBotInstruction(String(latest?.body ?? ""))
    if (!instruction) {
        await client.request("POST", `/repos/${owner}/${repo}/issues/${issueNumber}/comments`, {
            token,
            body: {
                body: `${BOT_MARKER}\n\nreview 需要修改，但没有找到 @dob-bot 指令`.trim(),
            },
        })
        return
    }
    ...
    const reviewTask = {
        ...
        instruction,
        context: String(latest?.body ?? ""),
    }

    const taskHash = computeTaskHash(reviewTask.source, reviewTask.instruction)
    const alreadyProcessed = await isTaskAlreadyProcessed({
        client,
        token,
        owner,
    ...
    await handleIssueTask({
        ...
        repo,
        issueNumber,
        issueTitle: `PR review #${state.prNumber}`,
        task: reviewTask,
        taskContext: String(latest?.body ?? ""),
        taskHash,
        existingState: state,
    })
}
