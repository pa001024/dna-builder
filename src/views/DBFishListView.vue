...
/**
 * 收起详情：清空钓鱼点与鱼的选择（保留当前列表类型）。
 */
function closeFishDetail(): void {
    selectedSpotId.value = 0
    selectedFishId.value = 0
}
...
<template>
    <div class="h-full flex flex-col">
        <SplitView
            :desktop-ratio="selectedSpot ? 1 / 3 : 1 / 2"
            :detail-open="Boolean(selectedSpot) || Boolean(selectedFish)"
            @collapse="closeFishDetail"
        >
            <template #master>
            <!-- 左侧列表面板 -->
            <div class="flex-1 flex min-h-0 flex-col overflow-hidden min-w-0" :class="{ 'sm:border-r border-base-content/10': selectedSpot || selectedFish }">
                <!-- 检索带：下划线搜索 + 计数 + 列表类型方章 -->
                <div
            ...
            <!-- 右侧：钓鱼池详情 -->
            <div v-if="selectedSpot" class="min-h-0 min-w-0 flex-2 overflow-hidden">
