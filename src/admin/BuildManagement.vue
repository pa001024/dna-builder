 ...
 * 构筑管理页配置
 */
const config: AdminCrudConfig<Build> = {
    title: "构筑管理",
    description: "管理用户构筑内容与互动状态",
    pageSize: 20,
    searchPlaceholder: "搜索构筑标题...",
    selectable: true,
    columns: [
        {
            key: "title",
    ...
    rowActions: [
        {
            ...
            async handler(item) {
                if (!item.isLiked) {
                    await likeBuildMutation({ id: item.id })
                    ui.showSuccessMessage("已点赞")
                    return
                }
                await unlikeBuildMutation({ id: item.id })
        ...
        },
        {
            ...
            tone: "neutral",
            async handler(item) {
                await recommendBuildMutation({ id: item.id, recommended: !item.isRecommended })
        ...
        },
        {
            ...
            tone: "neutral",
            async handler(item) {
                await pinBuildMutation({ id: item.id, pinned: !item.isPinned })
    ...
    bulkActions: [
        {
            ...
            async handler(items) {
                ...
                if (!(await ui.showDialog("批量删除确认", `确定要删除选中的 ${items.length} 条构筑吗？`))) {
                    return
                }

                await Promise.all(items.map(item => deleteBuildMutation({ id: item.id })))
                ui.showSuccessMessage(`已删除 ${items.length} 条构筑`)
            },
        },
    ],
    delete: {
        title: "删除确认",
        description: "此操作无法撤销，确定要删除这个构筑吗？",
        async run(item) {
            await deleteBuildMutation({ id: item.id })
