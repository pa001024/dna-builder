...
export class LevelUpCalculator {
    ...
    private static calculateSkillLevelUp(skill: Skill, currentLevel: number, targetLevel: number): ResourceCost {
        ...
        for (let level = currentLevel; level < targetLevel; level++) {
            LevelUpCalculator.mergeCost(skillCost, skill.升级[level])
        }
        return skillCost
    }
    ...
    private static calculateSingleModLevelUp(mod: Mod, config: ModLevelUpConfig): LevelUpResult {
        ...
        // 统一计算魔之楔升级消耗
        const totalCost = LevelUpCalculator.calculateModLevelUpCost(mod, currentLevel, targetLevel, count)
        const details = {
            levelUp: totalCost,
        }
    ...
    static getModDropDungeons(totalCost: ResourceCost) {
        ...
        // 辅助函数：获取副本时间 (分钟)
        const getDungeonTime = (d: Dungeon) => LevelUpCalculator.otherDungeonTime[d.t] || 1
    ...
    static estimateTime(totalCost: ResourceCost) {
        ...
        // 简化计算普通资源 普通资源来源单一
        for (const category of LevelUpCalculator.baseDungeonDropConfig) {
        ...
        // 计算魔之楔
        const modDungeonsWithCount = LevelUpCalculator.getModDropDungeons(totalCost)
...
interface WorkerResponse {
    ...
    result?:
        | LevelUpResult
        | ReturnType<typeof LevelUpCalculator.estimateTime>
        ...
        try {
            ...
            switch (method) {
                case "calculateCharLevelUp": {
                    ...
                    result = LevelUpCalculator.calculateCharLevelUp(charDataMsg.chars, charDataMsg.config)
                    break
                }
                case "calculateWeaponLevelUp": {
                    const weaponDataMsg = data as CalculateWeaponLevelUpData
                    result = LevelUpCalculator.calculateWeaponLevelUp(weaponDataMsg.weapons, weaponDataMsg.config)
                    break
                }
                case "calculateModLevelUp": {
                    const modDataMsg = data as CalculateModLevelUpData
                    result = LevelUpCalculator.calculateModLevelUp(modDataMsg.mods, modDataMsg.config)
                    break
                }
                case "estimateTime": {
                    const timeData = data as EstimateTimeData
                    result = LevelUpCalculator.estimateTime(timeData.totalCost)
                    break
                }
                case "mergeCalculate": {
                    // 处理合并计算请求
                    ...
                    // 计算角色养成结果
                    if (mergeData.chars) {
                        results.charResult = LevelUpCalculator.calculateCharLevelUp(mergeData.chars.chars, mergeData.chars.config)
                    }

                    // 计算武器养成结果
                    if (mergeData.weapons) {
                        results.weaponResult = LevelUpCalculator.calculateWeaponLevelUp(mergeData.weapons.weapons, mergeData.weapons.config)
                    }

                    // 计算魔之楔养成结果
                    if (mergeData.mods) {
                        results.modResult = LevelUpCalculator.calculateModLevelUp(mergeData.mods.mods, mergeData.mods.config)
                    }
