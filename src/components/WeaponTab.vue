...
<template>
    ...
    >
        ...
        <div class="space-y-1">
            ...
            />
            <FullTooltip v-for="[key, val] in Object.entries(weaponAttrs).filter(([k, v]) => v && k !== '多重')" :key="key" side="bottom">
                <template #tooltip>
                    <div class="flex flex-col gap-2">
                        ...
                        <ul class="space-y-1">
                            <li v-if="'基础' + key in baseWeapon" class="flex justify-between gap-8 text-sm text-primary">
                                ...
                                {{
                                    key !== "攻击"
                                        ? format100((baseWeapon as LeveledWeapon)[("基础" + key) as keyof LeveledWeapon]!)
