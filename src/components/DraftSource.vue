...
<template>
    <div v-if="props.draftSources.length > 0" class="space-y-2">
        ...
        <div class="grid grid-cols-1 gap-2">
            <div v-for="source in props.draftSources" :key="source.key" class="p-2 bg-base-200 rounded hover:bg-base-300 transition-colors">
                <div class="flex flex-col gap-2">
                    <div class="flex items-start justify-between gap-2 min-w-0">
                        ...
                        <span class="text-xs text-base-content/70 shrink-0">ID: {{ source.draft.id }}</span>
                    </div>
                    <div class="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-2">
                        ...
                        <template v-for="material in source.draft.x" :key="`${source.draft.id}:${material.t}:${material.id}`">
                            <ResourceCostItem :name="material.n" :value="material.t === 'Resource' ? material.c : [material.c, material.id, material.t]" />
                        </template>
                    </div>
                </div>
            </div>
