...
/** 魔灵显示开关：控制 RC 刷新点（魔灵点位与头像）是否绘制。 */
const showPetPoints = ref(true)
/** 资源显示开关：控制地图资源点位是否绘制。 */
const showResourcePoints = ref(true)
/** 魔灵筛选：已选中的筛选项 id 集合（pet-<id> 或 quality-<品质>），空集合 = 不过滤。 */
const selectedPetFilterIds = ref<Set<string>>(new Set())
...
interface StarfieldStar {
    ...
    /** 闪烁初始相位 */
    twinklePhase: number
    /** 视差系数（相对地图拖动的移动比例，层越近越大） */
    parallax: number
    /** 远近层：0 远（小暗）、1 中、2 近（大亮） */
    layer: 0 | 1 | 2
}

/**
 * 星云光晕数据（叠加在深空底色上、星星之下）。
 */
interface StarfieldNebula {
    x: number
    y: number
    radius: number
    /** RGB 三元组 */
...
/** 星空动画 rAF 句柄，0 表示未运行 */
let starfieldRaf = 0
/** 是否允许动画（尊重 prefers-reduced-motion） */
let starfieldMotionEnabled = false
/** 系统「减少动态效果」查询对象 */
let starfieldMotionQuery: MediaQueryList | null = null
/** 系统「减少动态效果」变化监听器 */
let starfieldMotionChangeHandler: ((event: MediaQueryListEvent) => void) | null = null
/** 星空布局缓存 key（按容器尺寸分桶，避免连续 resize 频繁重建） */
let starfieldCacheKey = ""
/** 星星数组（按当前尺寸缓存） */
let starfieldStars: StarfieldStar[] = []
/** 星云数组（按当前尺寸缓存） */
let starfieldNebula: StarfieldNebula[] = []
...
const isTeleportSectionCollapsed = ref(false)
const isLayerSectionCollapsed = ref(false)
const isPetSectionCollapsed = ref(false)
const isSubRegionSectionCollapsed = ref(false)
...
const petFilterOptions = computed<PetFilterOption[]>(() => {
    ...
    for (const subRegion of projectedSubRegions.value) {
        for (const rcInfo of subRegion.rcInfos) {
            for (const rate of rcInfo.petRates) {
                if (!seen.has(rate.petId)) seen.set(rate.petId, rate)
            }
        }
    }
    return [...seen.values()]
        .filter(rate => rate.petQuality === 5 && rate.petName.startsWith("闪亮"))
        .sort((a, b) => a.petId - b.petId)
        .map(rate => ({ id: `pet-${rate.petId}`, label: rate.petName, petId: rate.petId, iconUrl: rate.petIconUrl }))
})
const isPetFilterActive = computed(
    () => selectedPetFilterIds.value.size > 0 && selectedPetFilterIds.value.size === petFilterOptions.value.length
)
...
/**
 * 判断 RC 刷新点是否通过魔灵筛选。
 * 未选择任何筛选项或“全部”选中时视为不过滤，全部可见。
 */
function isRcInfoPetVisible(rcInfo: ProjectedRcInfo): boolean {
    const selected = selectedPetFilterIds.value
    if (selected.size === 0 || isPetFilterActive.value) return true
    return rcInfo.petRates.some(rate => selected.has(`pet-${rate.petId}`))
}

/**
 * 切换单个魔灵筛选项。
 * @param optionId 筛选项 id（pet-<实体id> 或 quality-<品质>）
 * @param checked 是否选中
 */
function togglePetFilter(optionId: string, checked: boolean) {
    const next = new Set(selectedPetFilterIds.value)
    if (checked) {
        next.add(optionId)
    } else {
        next.delete(optionId)
    }
    selectedPetFilterIds.value = next
}

/**
 * 处理魔灵筛选“全部”开关：全选则清空（等同不过滤），未全选则全部选中。
 * @param checked 是否选中全部
 */
function handlePetFiltersAllChange(checked: boolean) {
    selectedPetFilterIds.value = checked ? new Set(petFilterOptions.value.map(option => option.id)) : new Set()
}
...
function resetRegionSelectionState() {
    ...
    selectedPetFilterIds.value = new Set()
}
...
function drawSubRegionPoints(ctx: CanvasRenderingContext2D) {
    ...
    const visibleResourcePoints = !showResourcePoints.value
        ? []
        : isResourceFilterActive.value
...
/**
 * 按容器尺寸生成（或复用缓存的）星空数据：三层星星 + 星云光晕。
 * 尺寸以 100px 分桶，连续小幅 resize 不重建布局。
 */
function ensureStarfieldData(width: number, height: number) {
    const key = `${Math.round(width / 100)}x${Math.round(height / 100)}`
    if (key === starfieldCacheKey) return
    starfieldCacheKey = key

    const rand = mulberry32(0x5eed + width * 131 + height * 17)
    const starCount = Math.max(40, Math.min(420, Math.round((width * height) / 3200)))

    starfieldStars = []
    for (let i = 0; i < starCount; i++) {
        const layerRoll = rand()
        const layer: 0 | 1 | 2 = layerRoll < 0.55 ? 0 : layerRoll < 0.85 ? 1 : 2
        const radius = layer === 0 ? 0.25 + rand() * 0.4 : layer === 1 ? 0.55 + rand() * 0.35 : 0.85 + rand() * 0.45
        const baseAlpha = layer === 0 ? 0.25 + rand() * 0.35 : layer === 1 ? 0.45 + rand() * 0.4 : 0.7 + rand() * 0.25
        const twinkleSpeed = layer === 0 ? 0.4 + rand() * 0.8 : layer === 1 ? 0.8 + rand() * 1.2 : 1.2 + rand() * 1.8
        starfieldStars.push({
            x: rand() * width,
            y: rand() * height,
            radius,
            ...
            // 视差系数：远层 0.08、中层 0.16、近层 0.26，层越近跟随地图拖动的比例越大
            parallax: layer === 0 ? 0.08 : layer === 1 ? 0.16 : 0.26,
            layer,
        })
    }

    // 2-3 团星云光晕：紫 / 蓝 / 蓝紫 / 青
    const nebulaCount = 2 + Math.floor(rand() * 2)
    ...
    for (let i = 0; i < nebulaCount; i++) {
        starfieldNebula.push({
            x: rand() * width,
            y: rand() * height,
            radius: Math.min(width, height) * (0.35 + rand() * 0.45),
            color: nebulaColors[Math.floor(rand() * nebulaColors.length)],
...
/**
 * 绘制深空星空背景：径向渐变底 + 星云光晕 + 三层闪烁星星（随地图拖动视差移动）。
 * 尊重 prefers-reduced-motion：动画关闭时星星保持静态亮度（视差跟随拖动不受影响）。
 */
function drawStarfield(ctx: CanvasRenderingContext2D, width: number, height: number, panX: number, panY: number, now: number) {
    ensureStarfieldData(width, height)
    const animated = starfieldMotionEnabled

    // 深空径向渐变底：中心略亮（深蓝紫）向四周渐暗
    const bgGradient = ctx.createRadialGradient(width * 0.5, height * 0.42, 0, width * 0.5, height * 0.42, Math.hypot(width, height) * 0.75)
    bgGradient.addColorStop(0, "#151b3f")
    bgGradient.addColorStop(0.55, "#0b1029")
    bgGradient.addColorStop(1, "#070b1e")
    ctx.fillStyle = bgGradient
    ctx.fillRect(0, 0, width, height)

    // 星云光晕
    for (const nebula of starfieldNebula) {
        const glow = ctx.createRadialGradient(nebula.x, nebula.y, 0, nebula.x, nebula.y, nebula.radius)
        glow.addColorStop(0, `rgba(${nebula.color.join(",")},${nebula.alpha})`)
        glow.addColorStop(1, `rgba(${nebula.color.join(",")},0)`)
        ctx.fillStyle = glow
        ctx.fillRect(0, 0, width, height)
    }

    // 星星：正弦闪烁（静态模式固定亮度）+ 视差移动（跟随地图拖动，比例小于 1）
    const t = animated ? now / 1000 : 0
    for (const star of starfieldStars) {
        const twinkle = animated ? 0.55 + 0.45 * Math.sin(t * star.twinkleSpeed + star.twinklePhase) : 1
        const alpha = star.baseAlpha * (0.25 + 0.75 * twinkle)
        if (alpha <= 0.02) continue
        // 星星自身位置 + 视差偏移，取模回绕保证拖动后仍铺满屏幕
        const x = (((star.x + panX * star.parallax) % width) + width) % width
        const y = (((star.y + panY * star.parallax) % height) + height) % height
...
/**
 * 启动星空动画循环：每帧请求重绘，驱动星星闪烁。
 */
function startStarfieldLoop() {
    if (starfieldRaf || !starfieldMotionEnabled) return
    const tick = () => {
        requestDraw()
        starfieldRaf = requestAnimationFrame(tick)
    }
    starfieldRaf = requestAnimationFrame(tick)
}
...
/**
 * 初始化星空动画：读取系统「减少动态效果」偏好并监听其变化。
 * 动画偏好开启时启动 rAF 循环；关闭时仅绘制静态星空。
 */
function setupStarfieldMotion() {
    starfieldMotionQuery = window.matchMedia("(prefers-reduced-motion: reduce)")
    starfieldMotionEnabled = !starfieldMotionQuery.matches
    const handler = (event: MediaQueryListEvent) => {
        starfieldMotionEnabled = !event.matches
        if (starfieldMotionEnabled) {
            startStarfieldLoop()
        } else {
            stopStarfieldLoop()
            requestDraw() // 立即回到静态星空
        }
    }
    starfieldMotionChangeHandler = handler
    starfieldMotionQuery.addEventListener("change", handler)
    if (starfieldMotionEnabled) {
        startStarfieldLoop()
    }
}
 ...
 * 清理星空动画：停止循环并解绑监听。
 */
function teardownStarfieldMotion() {
    stopStarfieldLoop()
    const query = starfieldMotionQuery
    const handler = starfieldMotionChangeHandler
    if (query && handler) {
        query.removeEventListener("change", handler)
    }
    starfieldMotionQuery = null
    starfieldMotionChangeHandler = null
}
...
function drawScene() {
    ...
    // 深空星空背景（canvas 自绘，星星随地图拖动视差移动；尊重 prefers-reduced-motion）
    drawStarfield(ctx, width, height, panOffset.value.x, panOffset.value.y, performance.now())
...
watch(
    () => [
        ...
        [...selectedPetFilterIds.value].sort().join("|"),
    ...
    <div class="flex h-full min-h-0 bg-base-100">
        ...
        <section ref="containerRef" class="relative min-w-0 flex-1 overflow-hidden bg-[#070b1e]">
            ...
            <div class="absolute top-3 left-3 z-10 flex flex-col gap-1.5">
                ...
                >
                    <Icon :icon="isSidebarCollapsed ? 'tabler:arrow-bar-to-right' : 'tabler:arrow-bar-to-left'" class="size-4" />
                </button>
                <button
                    class="inline-flex size-8 cursor-pointer items-center justify-center rounded-xs border border-base-content/15 bg-base-100/85 text-base-content/70 backdrop-blur-xs transition-[border-color,color,transform] duration-150 hover:border-primary/50 hover:text-primary active:translate-y-px"
                    title="放大"
            ...
            <div class="absolute right-3 bottom-3 z-10 flex flex-col items-end gap-1.5">
                <div
                    v-if="hoverCoordinateInfo"
                    class="rounded-xs border border-base-content/15 bg-base-100/90 px-3 py-1.5 font-mono text-xs leading-5 shadow-sm backdrop-blur-sm"
                >
                    {{ hoverCoordinateInfo.worldX.toFixed(2) }}, {{ hoverCoordinateInfo.worldY.toFixed(2) }}
                </div>
                <div
                    class="rounded-xs border border-base-content/10 bg-base-100/80 px-2 py-0.5 text-[11px] font-medium text-base-content/60 tabular-nums backdrop-blur-sm"
                >
                    {{ (zoom * 100).toFixed(0) }}%
                </div>
            </div>
